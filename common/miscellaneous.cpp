...
void set_thread_name(pthread_t thread, const std::string& name){
    std::string truncated_name = name.substr(0, 15);
    int rc = pthread_setname_np(thread, truncated_name.c_str());
    if(rc != 0){
        RAISE_EXCEPTION(Exception, "[set_thread_name] error: " << strerror(errno) << " (" << errno << ")");
    }
}
...
void libevent_init(){
    ...
    if(g_libevent_active_clients > 0){
        ...
        if(rc != 0) RAISE_EXCEPTION(Exception, "[libevent_init] Cannot initialise the library");
...
size_t hyperfloor(size_t value) {
    return (size_t) pow(2, floor(log2(static_cast<double>(value))));
}
