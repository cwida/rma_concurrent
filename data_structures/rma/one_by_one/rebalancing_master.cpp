        ...
        case InternalTask::Type::TaskDone: {
            ...
            case RebalanceOperation::REBALANCE:
            {
                ...
                for(size_t i = rebal_task->get_lock_start(), end = rebal_task->get_lock_end(); i < end; i++){
                    release_lock(i, /* workspace */ worker_list);
                }
...
void RebalancingMaster::wait_to_complete_remove(RebalancingTask* task, uint64_t lock_id){
    ...
    if(it_wtc->m_cardinality != 0){
        ...
        // we can safely set gate.m_writer = nullptr here as well, still the protocol is that a worker
        // unsets this field before invoking the rebalancer
        assert(gate.m_writer == nullptr && "The write queue should have been unset by the writer invoking the rebalancer");
    }
...
void RebalancingMaster::rebal_resume(RebalancingTask* task){
    ...
    while(/*can_process &&*/ !do_rebalance && lock_length <= num_locks){
        ...
        COUT_DEBUG("(end iteration) cardinality: " << cardinality << ", lock_start: " << lock_start << ", lock_length: " << lock_length << ", capacity_per_lock: " << capacity_per_lock);
        COUT_DEBUG("height: " << height << ", rho: " << density_bounds.first << ", density: " << static_cast<double>(cardinality) / static_cast<double>(lock_length * capacity_per_lock) << ", tau: " << density_bounds.second << ","
                " cardinality_min: " << cardinality_min << ", cardinality: " << cardinality << ", cardinality_max: " << cardinality_max << " (tree height: " << m_instance->m_storage.hyperheight() << ")");
        ...
        if(cardinality_min <= cardinality && cardinality <= cardinality_max && !task->m_forced_resize /* => cardinality < capacity/2, that is we are wasting too much space, prefer a resize */) {
            do_rebalance = true;
        } else {
...
void RebalancingMaster::launch_task(RebalancingWorker* worker, RebalancingTask* task){
    ...
    debug_validate_launch_task(task);
...
uint64_t RebalancingMaster::acquire_lock(RebalancingTask* task, uint64_t lock_id){
    ...
    gate->m_writer = nullptr; // do not enqueue new items to insert/delete in the current writer's queue
...
void RebalancingMaster::release_lock(uint64_t lock_id, WakeList& worker_list){
    assert(lock_id < m_instance->get_number_locks() && "Invalid gate/lock ID");
    Gate* gate = m_instance->m_locks.get_unsafe() + lock_id;
    ...
    assert(gate->m_writer == nullptr && "Lock not released when it was acquired");
...
void RebalancingMaster::debug_validate_launch_task(RebalancingTask* task) const{
#if !defined(NDEBUG)
    assert(task != nullptr && "Null pointer");
    assert(task != nullptr && "Null pointer");
    ...
    if(cardinality_locks != cardinality_segments || cardinality_locks != task->m_plan.get_cardinality_after()){
        COUT_DEBUG_FORCE("--- CARDINALITY MISMATCH: task: " << task << ", cardinality gates: " << cardinality_locks << ", cardinality segments: " << cardinality_segments << ", cardinality plan: " << task->m_plan.get_cardinality_after());
    ...
    if(is_resize){
        ...
        if(m_instance->m_cardinality != task->m_plan.get_cardinality_after())
            COUT_DEBUG_FORCE("--- CARDINALITY MISMATCH RESIZE: task: " << task << ", cardinality plan: " << task->m_plan.get_cardinality_after() << ", cardinality pma: " << m_instance->m_cardinality);
        assert(m_instance->m_cardinality == task->m_plan.get_cardinality_after() && "Cardinality mismatch"); // as above
    }
#endif
}
