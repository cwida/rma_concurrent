...
Gate* Gate::allocate(uint64_t num_locks, uint64_t segments_per_lock){
    ...
    Gate* __restrict array_gates = (Gate*) malloc(space_per_gate * num_locks);
...
int64_t Gate::get_separator_key(size_t segment_id) const{
    assert(segment_id >= m_window_start && segment_id < m_window_start + m_window_length);
    if(segment_id == m_window_start)
        return m_fence_low_key;
    else
        return m_separator_keys[segment_id - m_window_start -1];
...
void Gate::wake_next(ClientContext* context) {
