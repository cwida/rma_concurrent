...
struct Storage {
    ...
    common::BufferedRewiredMemory* m_memory_keys = nullptr; // memory space used for the keys
    common::BufferedRewiredMemory* m_memory_values = nullptr; // memory space used for the values
    common::RewiredMemory* m_memory_sizes = nullptr; // memory space used for the segment cardinalities
    mutable ::common::SpinLock m_mutex; // used to protect rewiring by usage of multiple workers
...
public:
     ...
     */
    void alloc_workspace(size_t num_segments, int64_t** keys, int64_t** values, decltype(m_segment_sizes)* sizes, common::BufferedRewiredMemory** rewired_memory_keys, common::BufferedRewiredMemory** rewired_memory_values, common::RewiredMemory** rewired_memory_cardinalities);

    /**
     * Deallocate the space previously acquired with `alloc_workspace'
     */
    static void dealloc_workspace(int64_t** keys, int64_t** values, decltype(m_segment_sizes)* sizes, common::BufferedRewiredMemory** rewired_memory_keys, common::BufferedRewiredMemory** rewired_memory_values, common::RewiredMemory** rewired_memory_cardinalities);
