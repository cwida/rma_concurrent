...
//    size_t lastpos = 0; // debug only

//#if defined(DEBUG) // DEBUG ONLY
//    {
//        COUT_DEBUG("Segment cardinalities:");
//        auto& storage = m_task->m_pma->m_storage;
//        size_t num_segments = m_task->m_storage_num_extents * storage.get_segments_per_extent();
//        lastpos = (num_segments -1) * storage.m_segment_capacity + storage.m_segment_sizes[num_segments -1];
//        for(size_t i = 0; i < num_segments; i++){
//            size_t start, stop;
//            if(i % 2 == 0){
//                stop = (i+1) * storage.m_segment_capacity;
//                start = stop - storage.m_segment_sizes[i];
//            } else {
//                start = i * storage.m_segment_capacity;
//                stop = start + storage.m_segment_sizes[i];
//            }
//
//            COUT_DEBUG("[" << i << "] cardinality: " << m_task->m_pma->m_storage.m_segment_sizes[i] << ", start: " << start << " (incl.), stop: " << stop << " (excl.)") ;
//        }
//    }
//    {
...
//            cardinality += apma_partitions.cardinality_current();
//            apma_partitions.move(1);
//        }
//        assert(cardinality == m_task->m_plan.get_cardinality_after());
//    }
//#endif
...
//            COUT_DEBUG("cumulative cardinality: " << subtask_cardinality << ", position: " << position << ", element: " << m_task->m_pma->m_storage.m_keys[position]);
