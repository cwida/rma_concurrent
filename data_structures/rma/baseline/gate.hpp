...
public:
    ...
    struct SleepingBeauty{
        ...
        ThreadContext* m_thread; // the thread waiting
    };
...
public:
    ...
    /**
     * The call does nothing if the queue is empty, otherwise it wakes up a single writer or multiple readers from the queue.
     * Precondition: the caller holds the lock for this gate
     */
    void wake_next();

    /**
     * Wake all threads in the queue of this gate
     */
    void wake_all();
