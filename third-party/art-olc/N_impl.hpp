...
/*        do {
    ...
    N *N::setLeaf(Leaf* tid) {
        return reinterpret_cast<N *>(reinterpret_cast<uint64_t>(tid) | (1ull << 63));
    }

    Leaf* N::getLeaf(const N *n) {
        return (Leaf*) (reinterpret_cast<uint64_t>(n) & ((static_cast<uint64_t>(1) << 63) - 1));
    }
    ...
    Leaf* N::getAnyChildTid(const N *n, bool &needRestart) {
        const N *nextNode = n;
    ...
    Leaf* N::getMaxLeaf(N* node, uint64_t node_version, bool& needRestart){
        assert(node != nullptr);
//        std::cout << "[N::getMaxLeaf] node: " << node << std::endl;

        while(!isLeaf(node)){
            N* child = nullptr;
            switch (node->getType()) {
            case NTypes::N4:
                child = static_cast<N4*>(node)->getMaxChild();
                break;
            case NTypes::N16:
                child = static_cast<N16 *>(node)->getMaxChild();
                break;
            case NTypes::N48:
                child = static_cast<N48 *>(node)->getMaxChild();
                break;
            case NTypes::N256:
                child = static_cast<N256 *>(node)->getMaxChild();
//                std::cout << "[N256::getMaxLeaf] child: " << child << std::endl;
                break;
            }
            ...
            if(!isLeaf(child)){
                child_version = child->readLockOrRestart(needRestart); if(needRestart) return nullptr;
            }
    ...
    void N::dump(N* node, int level, int depth){
        using namespace std;
        assert(node != nullptr);
        ...
        if(isLeaf(node)){
            ...
            cout << "Leaf: " << node << ", key: " << tid->m_key << ", value: " << (uint64_t) tid->m_value << " (" << tid->m_value << ")\n";
        } else {
            ...
            cout << ", Version: " << node->typeVersionLockObsolete;
            ...
            for(uint32_t i = 0; i < childrenCount; i++){
                dump(get<1>(children[i]), level + 1 + node->getPrefixLength(), depth +1);
            }
        }
    }
}
