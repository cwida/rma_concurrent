...
using namespace data_structures::rma::batch_processing;
using namespace std;
...
TEST_CASE("sanity"){
    ...
    REQUIRE(pma.remove(10) == -1); // It should be 100, but this impl~ does not respect the return value
    REQUIRE(pma.size() == 0);
    REQUIRE(pma.empty());
...
TEST_CASE("single_thread_local_rebal"){
    ...
    for(int64_t i = 1; i <= sz; i++){
        pma.insert(i, i *10);
        pma.on_complete(); // sync behaviour

        // updates can be asynchronous, we cannot perform these checks now:
        REQUIRE(pma.size() == i);
    ...
    pma.on_complete(); // ensure the rebalancer has merged all updates
    REQUIRE(pma.size() == sz);
    ...
    for(int64_t i = 1; i <= sz; i++){
        REQUIRE(pma.find(i) == i * 10);
        pma.remove(i); // this impl~ always returns -1
    }
...
TEST_CASE("single_thread_global_rebal"){
    ...
    pma.on_complete(); // give some time to the rebalancer to terminate the insertions
...
//        pma.dump();
        REQUIRE(pma.find(i) == i * 10);
        pma.remove(i); // this impl~ always returns -1
//        REQUIRE(pma.size() == sz - i); // as the implementation is asynchronous
    }

    pma.on_complete(); // give some time to the rebalancer to terminate the deletions
...
TEST_CASE("multi_thread_local_rebal"){
    ...
    pma.on_complete(); // give some time to the rebalancer to ultimate the updates
...
TEST_CASE("multi_thread_global_rebal"){
    ...
    for(int i = 0; i < num_threads; i++){
        ...
        threads.emplace_back([&](int64_t pos_start, int64_t num_keys){
            ...
            for(int64_t pos = pos_start; pos < pos_end; pos++){
                int64_t key = sampler.get_raw_key(pos) +1;
                REQUIRE(pma.find(key) == key * 10); // the key must still be present
                pma.remove(key); // it always returns -1 in this implementation
...
//    constexpr int64_t num_elts = 10000000;
    ...
    PackedMemoryArray pma { /* block size */ 17, /* segment size */ 32, /* pages per extent */ 1, /* worker threads */ 4, /* segments per lock */ 4 };
    pma.set_max_number_workers(num_threads);
    ...
    for(int worker_id = 0; worker_id < num_threads; worker_id++){
        threads.emplace_back([&](int thread_id){
            ...
            while( (key = (current_key++)) <= num_elts){
                REQUIRE(pma.find(key) == -1); // the key must not be present yet
                pma.insert(key, key * 100);
            }
    ...
    for(auto& t : threads) t.join(); // Zzz
    pma.on_complete(); // give some time to the rebalancer to ultimate the insertions
    ...
    for(int worker_id = 0; worker_id < num_threads; worker_id++){
        threads.emplace_back([&](int worker_id){
            ...
            while( (key = current_key--) > 0 ){
                int64_t find_result = pma.find(key);
    ...
    for(auto& t : threads) t.join(); // Zzz
    pma.on_complete(); // give some time to the rebalancer to ultimate the deletions

    REQUIRE(pma.size() == 0);
    REQUIRE(pma.empty());
}

TEST_CASE("multi_thread_delayed"){
    data_structures::initialise();
    constexpr int num_threads = 8;
    constexpr int64_t num_elts = 10000000;
    static_assert(num_elts > 0, "The total amount of elements to insert must be a positive quantity");
...
TEST_CASE("sum_parallel"){
    ...
    pma.on_complete(); // let it complete all asynchronous updates
    REQUIRE(pma.size() == num_elts);
    for(size_t i = 1; i <= num_elts; i++){
        REQUIRE(pma.find(i) == i * 10);
    }
