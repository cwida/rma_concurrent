...
#include "data_structures/masstree/sequential.hpp"
#include "data_structures/iterator.hpp"

using namespace data_structures;
using namespace data_structures::masstree;
using namespace std;

TEST_CASE("sanity"){
    Sequential masstree{};
    REQUIRE(masstree.empty());
    ...
    int randomPermutation[] = {340, 980, 860, 900, 190, 110, 410, 490, 440, 330, 20, 680, 210, 970,
     ...
     * Insert
     */
    for(size_t i = 0; i < randomPermutation_sz; i++){
        auto key = randomPermutation[i];
        masstree.insert(key, key * 10);
        REQUIRE(masstree.find(key) == key * 10);
    }

    REQUIRE(masstree.size() == randomPermutation_sz);

    /**
     * Find
     */
    for(int i = 1; i < 1002; i++){
        auto v = masstree.find(i);
...
TEST_CASE("sum"){
    using Implementation = Sequential;
