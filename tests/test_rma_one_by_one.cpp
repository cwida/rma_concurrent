...
using namespace data_structures::rma::one_by_one;
using namespace std;
...
TEST_CASE("single_thread_local_rebal"){
    ...
    for(int64_t i = 1; i <= sz; i++){
        ...
        REQUIRE(pma.find(i) == -1); // the element doesn't exist anymore
    ...
    for(int worker_id = 0; worker_id < num_threads; worker_id++){
        threads.emplace_back([&](int worker_id){
            ...
            while( (key = current_key--) > 0 ){
                REQUIRE(pma.find(key) == key * 100); // the key is still present
