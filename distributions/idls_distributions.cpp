...
namespace distributions { namespace idls {
    ...
    struct Builder {
        ...
        Builder(eDistributionType init_distribution_type, double init_alpha, uint64_t init_seed,
            ...
            for(size_t i = 0; i < m_scan_ranges.size(); i++){
                ...
                if(alpha <= 0 || alpha > 1)
                    RAISE_EXCEPTION(common::Exception, "[IDLS_Builder] [" << i << "] Invalid value for alpha: " << alpha);
                if(num_repetitions == 0)
                    RAISE_EXCEPTION(common::Exception, "[IDLS_Builder] [" << i << "] Invalid value for num_repetitions: " << num_repetitions);
            }
        ...
        unique_ptr<Distribution<int64_t>> initialize_distribution(eDistributionType distribution_type, double alpha, size_t range, uint64_t seed){
            ...
            default:
                RAISE_EXCEPTION(common::Exception, "[IDLS_Builder] Invalid distribution: " << (int) distribution_type);
            }
        }
...
void Generator::set_initial_size(size_t value) {
    if(value == 0) RAISE_EXCEPTION(common::Exception, "The given value must be greater than 0: " << value);
    m_num_initial_inserts = value;
}

void Generator::set_insdel(size_t total, size_t group_size){
    if(group_size == 0) RAISE_EXCEPTION(common::Exception, "group_size == 0");
...
void Generator::set_scans(vector<pair<double, size_t>>& scan_vector){
    for(size_t i = 0; i < scan_vector.size(); i++){
        ...
        if(alpha <= 0 || alpha > 1)
            RAISE_EXCEPTION(common::Exception, "[" << i << "] Invalid value for the interval size: " << alpha << ". It must be a value in (0, 1]");
        int64_t num_repetitions = p.second;
        if(num_repetitions == 0)
            RAISE_EXCEPTION(common::Exception, "[" << i << "] Interval: " << alpha << ". Invalid value for the number of repetitions: 0");
    }
...
void Generator::set_distribution_type_init(eDistributionType value, double alpha){
    if(value == eDistributionType::zipf && (alpha <= 0)) // otherwise it's simply ignored
        RAISE_EXCEPTION(common::Exception, "[zipf distribution] Cannot set the parameter --alpha <= 0: " << alpha);
...
void Generator::set_distribution_type_insert(eDistributionType value, double alpha){
    if(value == eDistributionType::zipf && (alpha <= 0)) // otherwise it's simply ignored
        RAISE_EXCEPTION(common::Exception, "[zipf distribution] Cannot set the parameter --alpha <= 0: " << alpha);
//    if(beta < 1)
//        RAISE_EXCEPTION(common::Exception, "Cannot set the parameter --beta < 1: " << beta << ". It determines the window of generated values.");
...
DistributionsContainer Generator::generate() const {
    ...
    if(m_num_initial_inserts == 0)
        RAISE_EXCEPTION(common::Exception, "[Generator::generate] Number of initial inserts not set");
    if(m_distribution_range < 1 && (m_init_distribution_type == eDistributionType::zipf || m_insert_distribution_type == eDistributionType::zipf || m_delete_distribution_type == eDistributionType::zipf) ){
        RAISE_EXCEPTION(common::Exception, "Cannot set the parameter --beta < 1: " << m_distribution_range << ". It determines the window of generated values.");
    }
