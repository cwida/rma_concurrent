 ...
 */
class SequentialForward : public Interface {
private:
 ...
 */
class SequentialBackwards : public Interface {
private:
    const int64_t m_begin; // inclusive
    const int64_t m_end; // exclusive
 ...
 */
template <typename T>
class BulkDistribution : public Interface {
private:
...
template<typename T>
std::unique_ptr<Interface> BulkDistribution<T>::view(size_t start, size_t length) {
    if(start + length > m_capacity) throw std::invalid_argument("End interval out of bound");
    return std::unique_ptr<Interface>{new BulkDistribution<T>(*this, start, start + length)};
}
 ...
 */
template <typename T>
class InterleavedDistribution : public Interface {
private:
    std::shared_ptr<T>* m_slots;
    size_t m_slots_sz;
    size_t m_capacity;
...
template <typename T>
std::unique_ptr<Interface> InterleavedDistribution<T>::view(size_t start, size_t length) {
    if(start + length > m_capacity) throw std::invalid_argument("End interval out of bound");
    return std::unique_ptr<Interface>{new InterleavedDistribution(*this, start, start + length)};
}
 ...
 */
template<typename T>
class NoiseDistribution : public Interface {
private:
...
template <typename T>
std::unique_ptr<Interface> NoiseDistribution<T>::view(size_t start, size_t length) {
    if(start + length > m_capacity) throw std::invalid_argument("End interval out of bound");
    return std::unique_ptr<Interface>{new NoiseDistribution(*this, start, start + length)};
}
