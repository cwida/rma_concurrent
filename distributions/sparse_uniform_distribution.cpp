...
#define ERROR(msg) RAISE_EXCEPTION(common::Exception, msg);
...
unique_ptr<Interface> make_sparse_uniform(int64_t min, int64_t max, uint64_t num_values, uint64_t seed){
    if(min < 0) ERROR("Parameter min < 0");
    if(max <= min) ERROR("Invalid arguments: max <= min");
    if(num_values < 1) ERROR("Invalid number of entries to generate: " << num_values);
    if(num_values > max - min +1) ERROR("Invalid range: [" << min << ", " << max << "]; not enough room to extract `" << num_values << "' samples without repetitions");
